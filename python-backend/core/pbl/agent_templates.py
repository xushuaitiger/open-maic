"""Agent template prompts for PBL. Mirrors lib/pbl/mcp/agent-templates.ts"""

QUESTION_AGENT_EN = """You are a Question Agent in a Project-Based Learning platform. Your role is to help students understand and complete their assigned issue.
...
JUDGE_AGENT_EN = """You are a Judge Agent in a Project-Based Learning platform. Your role is to evaluate whether students have completed their assigned issue successfully.
...
QUESTION_AGENT_ZH = """你是项目式学习平台中的提问助手（Question Agent）。你的职责是帮助学生理解并完成分配给他们的任务。
...
JUDGE_AGENT_ZH = """你是项目式学习平台中的评判助手（Judge Agent）。你的职责是评估学生是否成功完成了分配的任务。
...
def get_question_agent_prompt(language: str = "en-US") -> str:
    return QUESTION_AGENT_ZH if language == "zh-CN" else QUESTION_AGENT_EN


def get_judge_agent_prompt(language: str = "en-US") -> str:
    return JUDGE_AGENT_ZH if language == "zh-CN" else JUDGE_AGENT_EN

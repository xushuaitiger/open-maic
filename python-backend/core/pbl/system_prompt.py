...
Mirrors lib/pbl/pbl-system-prompt.ts
"""
...
def build_pbl_system_prompt(config: PBLSystemPromptConfig) -> str:
    if config.language == "zh-CN":
        return _build_zh(config)
    return _build_en(config)


def _build_en(c: PBLSystemPromptConfig) -> str:
    skills = ", ".join(c.target_skills)
    return f"""You are a Teaching Assistant (TA) on a Project-Based Learning platform. You are fully responsible for designing group projects for students based on the course information provided by the teacher.
...
- **Project Topic**: {c.project_topic}
- **Project Description**: {c.project_description}
- **Target Skills**: {skills}
- **Suggested Number of Issues**: {c.issue_count}
...
3. Switch to **issueboard** mode: Create {c.issue_count} sequential issues that guide students through the project
...
- Create exactly {c.issue_count} issues that form a logical sequence
...
def _build_zh(c: PBLSystemPromptConfig) -> str:
    skills = "、".join(c.target_skills)
    return f"""你是项目式学习（PBL）平台的教学助手（TA）。你需要根据老师提供的课程信息，自主设计完整的学生小组项目。
...
老师提供的信息：
- **项目主题**：{c.project_topic}
- **项目描述**：{c.project_description}
- **目标技能**：{skills}
- **建议任务数量**：{c.issue_count}
...
3. 切换到 **issueboard** 模式：创建 {c.issue_count} 个顺序任务引导学生完成项目
